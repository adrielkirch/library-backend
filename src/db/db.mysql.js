...
async function getConnectionAsync(conn) {
...
(async () => {
  try {
    const connection = await getConnectionAsync(conn);
...
async function createManyRows(connection) {
  try {
    const numRows = await checkNumAuthorRows();
...
async function checkNumAuthorRows() {
  try {
